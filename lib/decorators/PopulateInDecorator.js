...
function PopulateInDecorator(cache, config) {
  ...
  this._listener = new ExpiryListener(cache._getClient(), {
...
PopulateInDecorator.prototype.setAccessedAt = function (key) {
  this._getStore().setAccessedAt(this._getDataKey(key),
    Date.now(), this._emitError);
};
...
PopulateInDecorator.prototype.setTrigger = function (key) {
  this._getClient().setTrigger(this._getTriggerKey(key),
    this._config.populateIn, this._emitError);
};
...
PopulateInDecorator.prototype.leasedPopulate = function (key, cb) {
  ...
  async.waterfall([
    store.getAccessedAt.bind(store, this._getDataKey(key)),
    ...
    function (accessedAt, cb) {
      ...
      self._cache.leasedPopulate(key, function (err) {
        ...
        // thus, we need to set the trigger manually in order to
        // continue polling
        if (err) self.setTrigger(key);
        cb(err);
      });
    }
  ], cb);
