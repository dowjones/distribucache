...
/**
 * Helper to call a `:before` event before a function
 * is called and `:after` right after the callback is
 * returned.
 *
 * @param {EventEmitter} emitt
 * @param {String} name - of the event
 * @param {Array} eventArgs - arguments passed to the event
 * @param {Function} func
 */

exports.eventWrap = function (emitter, name, eventArgs, func) {
  var before = Date.now();
  emitter.emit.apply(emitter, [name + ':before'].concat(eventArgs));
  return function afterWrapper(err) {
    var args;

    if (err) {
      args = [name + ':error', err].concat(eventArgs);
      emitter.emit.apply(emitter, args);
    }

    args = [name + ':after'].concat(eventArgs);
    args.push(Date.now() - before);
    emitter.emit.apply(emitter, args);

    func.apply(emitter, arguments);
  };
};
